#!/usr/bin/env python3
"""
HTTPS server for LearnMappers PWA
Serves with SSL/TLS for service worker support on network
"""
import http.server
import ssl
import socketserver
...
if __name__ == "__main__":
    ...
    if not os.path.exists(site_path):
        ...
        print(f"   - Use different site: SITE_DIR=sites/other-site python servers/serve-https.py")
        print(f"   - Or: python servers/serve-https.py sites/other-site")
        sys.exit(1)
    
    os.chdir(site_path)
    
    # SSL certificates are in project root
    ...
    if not os.path.exists(cert_file) or not os.path.exists(key_file):
        ...
        print(f"Expected at: {cert_file}")
        print("Run: mkcert localhost 127.0.0.1 ::1 $(ipconfig getifaddr en0)")
        sys.exit(1)
    ...
    with socketserver.TCPServer(("", port), Handler) as httpd:
        ...
        print(f"\n{'='*60}")
        print(f"LearnMappers PWA HTTPS Server")
        print(f"{'='*60}")
        ...
        print(f"   (certificate is self-signed but trusted via mkcert)")

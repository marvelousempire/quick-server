...
// Wait for CONFIG to be available, then load schemas
(function loadSchemas() {
  if (!window.CONFIG) {
    // Config not ready yet, retry
    setTimeout(loadSchemas, 100);
    return;
  }
  
  (async function() {
    // Try to get schema paths from config, or use default paths
    const schemas = window.CONFIG?.schemas || {};
    const schemaNames = ['resource', 'service', 'person', 'company', 'relationship'];
    
    // Default schema paths (relative to current page location)
    // From inventory.html (content/pages/), schemas are at ../../schemas/
    // From validator.js (schemas/), schemas are in the same directory
    const currentPath = window.location.pathname;
    const isInventoryPage = currentPath.includes('inventory.html');
    const basePath = isInventoryPage ? '../../schemas/' : '';
    
    const defaultPaths = {
      'resource': basePath + 'resource-schema.json',
      'service': basePath + 'service-schema.json',
      'person': basePath + 'person-schema.json',
      'company': basePath + 'company-schema.json',
      'relationship': basePath + 'relationship-schema.json'
    };
    
    for (const name of schemaNames) {
      const schemaConfig = schemas[name];
      const schemaPath = schemaConfig?.path || defaultPaths[name];
      
      if (schemaPath) {
        try {
          const loaded = await window.validator.loadSchema(name, schemaPath);
          if (loaded) {
            console.log(`✅ ${name} schema loaded from ${schemaPath}`);
          } else {
            console.warn(`⚠️ Failed to load ${name} schema from ${schemaPath}`);
          }
        } catch (e) {
          console.error(`❌ Error loading ${name} schema:`, e);
        }
      }
    }
    
    // Log loaded schemas
    const loadedSchemas = Object.keys(window.validator.schemas);
    console.log(`📋 Loaded schemas: ${loadedSchemas.join(', ') || 'none'}`);
    
    // Dispatch event when schemas are loaded
    window.dispatchEvent(new CustomEvent('schemasLoaded', { 
      detail: { schemas: loadedSchemas } 
    }));
  })();
})();

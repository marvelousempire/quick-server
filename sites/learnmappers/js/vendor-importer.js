...
class VendorImporter {
  ...
  async importCSV(text, filename) {
    ...
    if (text.charCodeAt(0) === 0xFEFF) {
      text = text.slice(1);
      console.log('Auto-Know: Stripped UTF-8 BOM from CSV');
    }
    ...
    console.log('Auto-Know: CSV headers detected:', headers.slice(0, 10), '... (total:', headers.length, 'columns)');
    ...
    console.log(`Auto-Know: Using ${vendorType} mapper for ${rows.length - 1} data rows`);
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      try {
        ...
        if (resource && resource.title) {
          // Auto-Fit: Validate against schema (only if schema is loaded)
          if (this.validator && this.validator.schemas && this.validator.schemas['resource']) {
            ...
            // Schema not loaded yet - skip validation but log warning
            if (this.validator && !this.validator.schemas['resource']) {
              console.warn('⚠️ Resource schema not loaded - skipping validation for row', i + 1);
            }
          }
  ...
  async importZIP(file) {
    ...
    // If no CSV/TXT, look for HTML purchase history (eBay reports)
    if (!csvFile) {
      const htmlFile = Object.values(zip.files).find(f => 
        /purchasehistory\.html$/i.test(f.name) && !f.dir
      );
      
      if (htmlFile) {
        console.log('Auto-Know: Found HTML purchase history file:', htmlFile.name);
        return await this.importHTML(htmlFile, file.name);
      }
    }

    if (!csvFile) {
      throw new Error(`No CSV, TXT, or HTML purchase history file found in ZIP. Found files: ${allFiles.join(', ')}`);
    }
    ...
    // Add source file to each result for vendor detection
    if (result.results) {
  ...
  /**
   * Import HTML file (eBay purchase history, etc.)
   */
  async importHTML(htmlFile, sourceFileName) {
    console.log('Auto-Know: Processing HTML file:', htmlFile.name);
    
    const htmlText = await htmlFile.async('string');
    console.log('Auto-Know: HTML extracted,', htmlText.length, 'characters');
    
    // Detect vendor from HTML content
    const vendorType = this.detectVendorTypeFromHTML(htmlText, htmlFile.name);
    console.log('Auto-Know: Vendor type detected from HTML:', vendorType);
    
    const mapper = this.getVendorMapper(vendorType);
    const resources = [];
    
    // Parse HTML table
    const tableMatch = htmlText.match(/<table[^>]*>(.*?)<\/table>/is);
    if (!tableMatch) {
    ...
    if (!rowMatches || rowMatches.length < 2) {
      throw new Error('No data rows found in HTML table');
    }
    ...
    for (let i = 1; i < rowMatches.length; i++) {
      ...
      try {
        // Check if mapper has mapHTML method (for eBay)
        let resource;
        if (vendorType === 'ebay' && window.EbayMapper) {
          const ebayMapper = new window.EbayMapper();
          resource = ebayMapper.mapHTML(row);
        } else {
          // Fallback: try to extract data from HTML row
          const cells = row.match(/<td[^>]*>(.*?)<\/td>/gi) || [];
          const data = cells.map(cell => cell.replace(/<[^>]+>/g, '').trim());
          resource = this.mapGenericHTML(data, vendorType);
        }
        ...
        if (resource && resource.title) {
          ...
          // Auto-Heal: Normalize resource
          resource = this.normalizeResource(resource);
          resource._sourceFile = sourceFileName;
    ...
    console.log('Auto-Know: HTML import complete. Resources:', resources.length, 'Errors:', this.errors.length);
  ...
  /**
   * Detect vendor type from HTML content
   */
  detectVendorTypeFromHTML(htmlText, filename) {
    const text = htmlText.toLowerCase();
    const filenameLower = filename.toLowerCase();
    
    if (text.includes('ebay') || filenameLower.includes('ebay') || text.includes('purchase history')) {
      return 'ebay';
    }
  ...
  /**
   * Map generic HTML row data
   */
  mapGenericHTML(data, vendorType) {
    return {
      title: data[0] || data[2] || 'Untitled Item',
      name: data[0] || data[2] || 'Untitled Item',
      category: 'other',
      type: 'other',
      description: data[2] || data[0] || '',
      purpose: `Imported from ${vendorType}`,
      meta: {
        purchasePrice: this.parsePrice(data[3] || data[6]),
        purchaseDate: this.parseDate(data[0]),
        quantity: data[4] ? parseInt(data[4], 10) : 1,
        vendor: vendorType
      },
      status: 'available',
      _vendor: vendorType,
      _source: `${vendorType}-import`
    };
  }

  /**
   * Auto-Know: Detect vendor type from headers and content (enhanced detection)
   * Uses dedicated mapper classes for better detection
   */
  detectVendorType(headers, filename) {
    ...
    // Try dedicated mappers first (if loaded)
    const mappers = [
      { name: 'amazon', class: window.AmazonMapper },
      { name: 'ebay', class: window.EbayMapper },
      { name: 'homedepot', class: window.HomeDepotMapper }
    ];

    for (const mapper of mappers) {
      if (mapper.class && typeof mapper.class === 'function') {
        const mapperInstance = new mapper.class();
        if (mapperInstance.detect && mapperInstance.detect(headers, filename)) {
          console.log(`Auto-Know: Detected ${mapper.name} format (using dedicated mapper)`);
          return mapper.name;
        }
      }
    }

    // Fallback to legacy detection if mappers not loaded
    if (filenameLower.includes('amazon') || 
      ...
      console.log('Auto-Know: Detected Amazon format (legacy)');
      return 'amazon';
    }
    ...
    if (filenameLower.includes('ebay') || 
      ...
      console.log('Auto-Know: Detected eBay format (legacy)');
      return 'ebay';
    }
    ...
    if (filenameLower.includes('home depot') || 
      ...
      console.log('Auto-Know: Detected Home Depot format (legacy)');
      return 'homedepot';
    }
   ...
   * Uses dedicated mapper classes if available, falls back to legacy methods
   */
  getVendorMapper(vendorType) {
    // Try to use dedicated mapper classes first
    const mapperClasses = {
      amazon: window.AmazonMapper,
      ebay: window.EbayMapper,
      homedepot: window.HomeDepotMapper
    };

    if (mapperClasses[vendorType] && typeof mapperClasses[vendorType] === 'function') {
      const mapperInstance = new mapperClasses[vendorType]();
      if (mapperInstance.map) {
        console.log(`Using dedicated ${vendorType} mapper class`);
        return mapperInstance.map.bind(mapperInstance);
      }
    }

    // Fallback to legacy mappers
    const legacyMappers = {
    ...
    console.log(`Using legacy ${vendorType} mapper`);
    return legacyMappers[vendorType] || legacyMappers.generic;
  }
  ...
  mapAmazon(headers, row) {
    ...
    // Extract transaction/order ID from various possible fields
    const transactionId = get([
      ...
      'receipt number', 'confirmation number', 'amazon order id', 'order-id'
    ]);
    ...
    return {
      ...
      meta: {
        ...
        orderId: transactionId || undefined, // Also store as orderId for compatibility
  ...
  mapHomeDepot(headers, row) {
    ...
    const brand = get(['brand', 'manufacturer', 'vendor']);
    
    // Extract transaction/order ID from various possible fields
    const transactionId = get([
      'order id', 'order number', 'transaction id', 'transaction number',
      'purchase id', 'order id', 'order#', 'transaction#', 'invoice number',
      'receipt number', 'confirmation number', 'receipt id'
    ]);
